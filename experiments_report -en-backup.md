...
*   **IDF1**: ID F1 Score. Measures how consistently the tracker follows the same object.
*   **IDP / IDR**: ID Precision and ID Recall.
*   **Rcll (Recall)**: Percentage of ground truth objects correctly detected.
*   **Prcn (Precision)**: Percentage of detected objects that are real.
*   **GT**: Ground Truth. Total number of unique objects in the video.
*   **MT (Mostly Tracked)**: Tracks that cover >80% of the ground truth trajectory.
*   **PT (Partially Tracked)**: Tracks that cover 20-80% of the ground truth trajectory.
*   **ML (Mostly Lost)**: Tracks that cover <20% of the ground truth trajectory.
*   **FP (False Positives)**: Number of false detections (ghosts).
*   **FN (False Negatives)**: Number of missed detections.
*   **IDs (ID Switches)**: Number of times the tracker mistakenly changes the ID of an object.
*   **FM (Fragmentation)**: Number of times a track is interrupted and resumed.
*   **MOTA**: Multiple Object Tracking Accuracy. Overall performance metric accounting for FP, FN, and IDs.
*   **MOTP**: Multiple Object Tracking Precision. Average overlap between predicted and ground truth boxes.
*   **IDt (ID Transfer)**: Number of times a track ID is transferred to a different ground truth object.
*   **IDa (ID Ascend)**: Number of times a tracker ID is assigned to a ground truth object previously tracked by a different ID.
*   **IDm (ID Migrate)**: Number of times a tracker ID switches to a ground truth object while the previous ID is still active.
...
The baseline Norfair implementation shows relatively low performance with an IDF1 of 30.3% and MOTA of 21.4%. The high number of False Negatives (67,143) suggests the tracker misses many objects, likely due to the simple distance-based matching or the confidence threshold.
...
DeepSORT improves IDF1 (37.4%) and Recall compared to Norfair but suffers a significant drop in MOTA (12.3%). This is driven by a massive increase in False Positives (33,183) and ID switches (573), indicating the tracker is maintaining tracks too long or creating ghosts.

---

## 3. OC-SORT Baseline 1 
...
The baseline OC-SORT configuration provides a stable MOTA (22.3%) comparable to Norfair but with significantly fewer False Positives (1,608) and ID switches (213). However, Recall remains low (24.3%).

---

## 4. OC-SORT Baseline 2 
...
Results are effectively identical to Baseline 1, confirming consistency in the baseline configuration.
...
Tuning hyperparameters (lower confidence threshold, larger inference size) yielded a major improvement. IDF1 jumped to 43.5% and MOTA to 35.8%. Recall nearly doubled to 43.8%, though ID switches increased (585) due to more active tracks.

---

## 6. OC-SORT + ByteTrack (High Conf 0.6) 
...
Adding ByteTrack logic with a high confidence threshold of 0.6 reduced ID switches significantly to 204 (from 585 in Tuned). IDF1 improved to 45.4%, showing better identity consistency.

---

## 7. OC-SORT + ByteTrack (High Conf 0.4) 
...
Lowering the high confidence threshold to 0.4 increased Recall (43.5%) and MOTA (36.1%) slightly, but at the cost of doubling ID switches (432) and increasing False Positives.
...
Increasing `MAX_AGE` to 60 slightly improved IDF1 to 46.3%, helping to maintain tracks through longer occlusions without negatively impacting other metrics significantly.

---

## 9. OC-SORT + ByteTrack + Interpolation 
...
Linear interpolation provided a solid boost, raising IDF1 to 48.5% and MOTA to 38.9%. Recall crossed 50% for the first time, showing that filling gaps in tracks is highly effective.
...
Adding Camera Motion Compensation (CMC) resulted in the best performance yet. IDF1 reached 51.7% and MOTA 40.4%. ID switches dropped to 276, proving that accounting for camera movement is crucial for this dataset.
...
Increasing the inference size to 1920px resulted in only marginal improvements across most metrics (MOTA +0.3%, IDF1 +0.4%) compared to the 1280px baseline (Experiment 10). Given the significantly higher computational cost of processing larger images, the performance gain is likely not worth the trade-off.
---
...
Increasing inference size to 1920px offered marginal gains over 1280px (IDF1 52.1% vs 51.7%), suggesting diminishing returns for the increased computational cost.

---

## 13. OC-SORT + ByteTrack + GSI + CMC Optimized 
...
Aggressive optimization (lower confidence, higher max age) backfired slightly, increasing False Positives (12,030) and ID switches (447), lowering MOTA to 39.2%.

---

## 14. OC-SORT + ByteTrack + GSI + CMC Optimized 2 
...
This configuration achieved the highest IDF1 (52.9%) of all experiments. It strikes a good balance between Recall (51.7%) and Precision, with stable MOTA (40.3%).
...
SAM2 performed poorly for this tracking task, with a negative MOTA (-46.2%) and very low IDF1 (13.9%). The high number of False Positives suggests the segmentation model is detecting background or non-target objects as targets.
...
Finetuning the model for Norfair actually degraded performance (MOTA 14.5%) compared to the baseline (MOTA 21.4%). This suggests potential overfitting or that the default YOLO weights were already quite robust for these classes.

---

## 17. Trained Best Tracker 
...
Applying the finetuned model to the best tracker configuration (OC-SORT+ByteTrack+CMC) resulted in significantly worse performance (MOTA 17.3%) than using the standard model (MOTA 40.4%), confirming the finetuning process was detrimental.
...
Consistent with other trained experiments, the trained OC-SORT model underperformed the standard YOLO model (MOTA 18.8% vs 22.3% baseline), though it performed slightly better than the trained Norfair setup.
