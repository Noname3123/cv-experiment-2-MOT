# Izvještaj o eksperimentu

Ovaj dokument sažima rezultate 18 eksperimenata praćenja provedenih na MOT17 skupu podataka.

## Ključ metrika

*   **IDF1**: ID F1 rezultat. Mjeri koliko dosljedno pratitelj prati isti objekt.
*   **IDP / IDR**: ID preciznost i ID odziv.
*   **Rcll (Odziv)**: Postotak objekata iz stvarne istine (ground truth) koji su ispravno detektirani.
*   **Prcn (Preciznost)**: Postotak detektiranih objekata koji su stvarni.
*   **GT**: Stvarna istina (Ground Truth). Ukupan broj jedinstvenih objekata u videu.
*   **MT (Uglavnom praćeni)**: Tragovi koji pokrivaju >80% putanje stvarne istine.
*   **PT (Djelomično praćeni)**: Tragovi koji pokrivaju 20-80% putanje stvarne istine.
*   **ML (Uglavnom izgubljeni)**: Tragovi koji pokrivaju <20% putanje stvarne istine.
*   **FP (Lažno pozitivni)**: Broj lažnih detekcija (duhovi).
*   **FN (Lažno negativni)**: Broj propuštenih detekcija.
*   **IDs (ID zamjene)**: Broj puta kada pratitelj pogrešno promijeni ID objekta.
*   **FM (Fragmentacija)**: Broj puta kada je trag prekinut i nastavljen.
*   **MOTA**: Točnost praćenja više objekata (Multiple Object Tracking Accuracy). Ukupna metrika performansi koja uzima u obzir FP, FN i ID-ove.
*   **MOTP**: Preciznost praćenja više objekata (Multiple Object Tracking Precision). Prosječno preklapanje između predviđenih okvira i okvira stvarne istine.
*   **IDt (ID prijenos)**: Broj puta kada je ID traga prenesen na drugi objekt stvarne istine.
*   **IDa (ID uspon)**: Broj puta kada je ID pratitelja dodijeljen objektu stvarne istine koji je prethodno praćen drugim ID-om.
*   **IDm (ID migracija)**: Broj puta kada ID pratitelja pređe na objekt stvarne istine dok je prethodni ID još aktivan.
...
| Sekvenca | IDF1 | IDP | IDR | Rcll | Prcn | GT | MT | PT | ML | FP | FN | IDs | FM | MOTA | MOTP | IDt | IDa | IDm |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
...
Osnovna implementacija Norfaira pokazuje relativno niske performanse s IDF1 od 30,3% i MOTA od 21,4%. Visok broj lažno negativnih rezultata (67.143) sugerira da pratitelj propušta mnoge objekte, vjerojatno zbog jednostavnog podudaranja na temelju udaljenosti ili praga pouzdanosti.
...
**Konfiguracija:**
*   **Model:** yolo11x.pt
*   **Pratitelj:** DeepSORT (deep-sort-realtime)
*   **Hiperparametri:**
    *   CONFIDENCE_THRESHOLD: 0.5
    *   INFERENCE_SIZE: 1920
    *   MAX_AGE: 60
    *   MIN_HITS: 3
    *   POST_PROCESSING: Linearna interpolacija (max_gap=20)
    *   CMC: NIJE primijenjeno
...
DeepSORT poboljšava IDF1 (37,4%) i odziv u usporedbi s Norfairom, ali pati od značajnog pada u MOTA (12,3%). To je uzrokovano masovnim povećanjem lažno pozitivnih rezultata (33.183) i zamjena ID-ova (573), što ukazuje da pratitelj predugo održava tragove ili stvara duhove.

---

## 3. OC-SORT Osnovna linija 1
...
Osnovna konfiguracija OC-SORT-a pruža stabilnu MOTA (22,3%) usporedivu s Norfairom, ali sa znatno manje lažno pozitivnih rezultata (1.608) i zamjena ID-ova (213). Međutim, odziv ostaje nizak (24,3%).

---

## 4. OC-SORT Osnovna linija 2
...
Rezultati su zapravo identični osnovnoj liniji 1, potvrđujući dosljednost u osnovnoj konfiguraciji.
...
Podešavanje hiperparametara (niži prag pouzdanosti, veća veličina inferencije) donijelo je veliko poboljšanje. IDF1 je skočio na 43,5%, a MOTA na 35,8%. Odziv se gotovo udvostručio na 43,8%, iako su se zamjene ID-ova povećale (585) zbog aktivnijih tragova.

---

## 6. OC-SORT + ByteTrack (Visoka pouzdanost 0.6)

**Konfiguracija:**
*   **Model:** yolo11l.pt
*   **Pratitelj:** OC-SORT + ByteTrack logika
*   **Hiperparametri:**
...
Dodavanje logike ByteTrack s visokim pragom pouzdanosti od 0,6 značajno je smanjilo zamjene ID-ova na 204 (s 585 u podešenom). IDF1 se poboljšao na 45,4%, pokazujući bolju dosljednost identiteta.

---

## 7. OC-SORT + ByteTrack (Visoka pouzdanost 0.4)
...
Smanjenje visokog praga pouzdanosti na 0,4 neznatno je povećalo odziv (43,5%) i MOTA (36,1%), ali po cijenu udvostručenja zamjena ID-ova (432) i povećanja lažno pozitivnih rezultata.

---

## 8. OC-SORT + ByteTrack (Max Age 60)
...
Povećanje `MAX_AGE` na 60 neznatno je poboljšalo IDF1 na 46,3%, pomažući u održavanju tragova kroz dulja zaklanjanja bez značajnog negativnog utjecaja na druge metrike.

---

## 9. OC-SORT + ByteTrack + Interpolacija

**Konfiguracija:**
*   **Model:** yolo11x.pt
*   **Pratitelj:** OC-SORT + ByteTrack logika + Interpolacija
*   **Hiperparametri:**
    *   CONFIDENCE_THRESHOLD (High): 0.4
    *   CONFIDENCE_LOW: 0.05
    *   INFERENCE_SIZE: 1280
...
Linearna interpolacija pružila je solidan poticaj, podižući IDF1 na 48,5% i MOTA na 38,9%. Odziv je prvi put prešao 50%, pokazujući da je popunjavanje praznina u tragovima vrlo učinkovito.

---

## 10. OC-SORT + ByteTrack + Interp + CMC

**Konfiguracija:**
*   **Model:** yolo11x.pt
*   **Pratitelj:** OC-SORT + ByteTrack + Interpolacija + CMC
*   **Hiperparametri:**
...
Dodavanje kompenzacije kretanja kamere (CMC) rezultiralo je najboljim performansama do sada. IDF1 je dosegao 51,7%, a MOTA 40,4%. Zamjene ID-ova pale su na 276, dokazujući da je uzimanje u obzir kretanja kamere ključno za ovaj skup podataka.

---

## 11. OC-SORT + ByteTrack + Interp + CMC (1920px)
...
Povećanje veličine inferencije na 1920px rezultiralo je samo marginalnim poboljšanjima u većini metrika (MOTA +0,3%, IDF1 +0,4%) u usporedbi s osnovnom linijom od 1280px (Eksperiment 10). S obzirom na znatno veći računalni trošak obrade većih slika, dobitak u performansama vjerojatno nije vrijedan kompromisa.

---

## 12. OC-SORT + ByteTrack + GSI + CMC

**Konfiguracija:**
*   **Model:** yolo11x.pt
*   **Pratitelj:** OC-SORT + ByteTrack + GSI (Gaussovo zaglađivanje) + CMC
*   **Hiperparametri:**
    *   CONFIDENCE_THRESHOLD (High): 0.5
    *   CONFIDENCE_LOW: 0.05
    *   INFERENCE_SIZE: 1920
    ...
    *   MIN_HITS: 3
    *   INERTIA: 0.2
    *   POST_PROCESSING: Gaussovo zaglađena interpolacija (sigma=1.0)
    *   CMC: Optički tok (Afini)
...
Povećanje veličine inferencije na 1920px ponudilo je marginalne dobitke u odnosu na 1280px (IDF1 52,1% naspram 51,7%), sugerirajući smanjenje povrata za povećani računalni trošak.

---

## 13. OC-SORT + ByteTrack + GSI + CMC Optimizirano
...
*   **Hiperparametri:**
    ...
    *   MIN_HITS: 3
    *   INERTIA: 0.2
    *   POST_PROCESSING: Gaussovo zaglađena interpolacija (sigma=0.6)
    *   CMC: Optički tok (Afini)
...
Agresivna optimizacija (niža pouzdanost, veća maksimalna dob) malo se osvetila, povećavajući lažno pozitivne rezultate (12.030) i zamjene ID-ova (447), snižavajući MOTA na 39,2%.

---

## 14. OC-SORT + ByteTrack + GSI + CMC Optimizirano 2
...
Ova konfiguracija postigla je najviši IDF1 (52,9%) od svih eksperimenata. Postiže dobru ravnotežu između odziva (51,7%) i preciznosti, uz stabilnu MOTA (40,3%).
...
SAM2 je pokazao loše rezultate za ovaj zadatak praćenja, s negativnom MOTA (-46,2%) i vrlo niskim IDF1 (13,9%). Visok broj lažno pozitivnih rezultata sugerira da model segmentacije detektira pozadinu ili neciljane objekte kao ciljeve.

---

## 16. Trenirani Norfair

**Konfiguracija:**
*   **Model:** yolo11l.pt (Fino podešen na MOT17 svim klasama)
*   **Pratitelj:** Norfair
*   **Hiperparametri:**
    *   EPOCHS: 10
    *   IMG_SIZE: 640
...
Fino podešavanje modela za Norfair zapravo je pogoršalo performanse (MOTA 14,5%) u usporedbi s osnovnom linijom (MOTA 21,4%). To sugerira potencijalno prekomjerno prilagođavanje (overfitting) ili da su zadane YOLO težine već bile prilično robusne za ove klase.

---

## 17. Trenirani Najbolji Pratitelj
...
Primjena fino podešenog modela na najbolju konfiguraciju pratitelja (OC-SORT+ByteTrack+CMC) rezultirala je znatno lošijim performansama (MOTA 17,3%) od korištenja standardnog modela (MOTA 40,4%), potvrđujući da je proces finog podešavanja bio štetan.

---

## 18. Trenirani OC-SORT

**Konfiguracija:**
*   **Model:** yolo11l.pt (Fino podešen na MOT17 svim klasama)
*   **Pratitelj:** OC-SORT (Prilagođena implementacija)
*   **Hiperparametri:**
    *   EPOCHS: 10
    *   IMG_SIZE: 640
    *   BATCH_SIZE: 4
...
U skladu s drugim eksperimentima s treniranjem, trenirani OC-SORT model bio je lošiji od standardnog YOLO modela (MOTA 18,8% naspram 22,3% osnovne linije), iako je bio nešto bolji od trenirane Norfair postavke.

---

## 19. Zaključak

Analiza provedenih eksperimenata pokazuje da OC-SORT pratitelj proširen ByteTrack logikom, linearnom interpolacijom i kompenzacijom kretanja kamere (CMC) je balansiran između performansi i kvalitete modela (među trackerima koji koriste najveći model).

Iako je konfiguracija s YOLO11x modelom i veličinom inferencije od 1920px (Eksperiment 11) postigla najbolje ukupne metrike (MOTA 40,7%, IDF1 52,1%), nudi marginalna poboljšanja u odnosu na alternativu od 1280px (Eksperiment 10, MOTA 40,4%). S obzirom na to da rezolucija od 1920px značajno povećava memorijske zahtjeve, model od 1280px nudi bolju učinkovitost uz zanemariv pad točnosti (-0,3% MOTA).

Dodatno, uvođenje kompenzacije kretanja kamere (CMC) pokazalo se ključnim za smanjenje zamjena ID-ova, dok su standardne YOLO težine nadmašile fino podešene modele, što naglašava bolju generalizaciju osnovnog modela.
